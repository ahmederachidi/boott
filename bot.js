...
message.channel.send(`**:white_check_mark: ${user.tag} banned from the server ! :airplane: **  `)
  ...
  if(!message.guild.member(client.user).hasPermission("KICK_MEMBERS")) return message.reply("```#kick @Torgut``` **يجب تحديد شخص** KICK_MEMBERS ` Permission**");
  let user = message.mentions.users.first();
  
  if (message.mentions.users.size < 1) return message.reply("```#kick @Torgut``` **يجب تحديد شخص**");
  if (!message.guild.member(user)
  .bannable) return message.reply("**تكون رتبة البوت اعلي من رتبه الشخص المراد طرده**");
...
////ثاني شي يمنع نشره وتعديل الحقوق وتقول الكود انا اللي مسوييه او مدري ايش
////ثالث شي لاي استفسار انا موجود علي الديسكورد اسمي Reverse#0505
////الكود متعوب عليه الف 
////ما تجي تبيعه بأي سيرفر وتقول انا اللي مسويه ومدري ايش تبي تبيعه تعال ديسكورد
...
message.channel.send('**لديك 10 ثانية لكتابة الكلمة**').then(msg => {
	let embed = new Discord.RichEmbed()
	.setColor('#000000')
	.setFooter("لرؤية مجموع نقاطك اكتب ^قاطي")
...
client.on('message', msg => {
  if (msg.content === prefix +'join') {
    msg.reply('https://discordapp.com/api/oauth2/authorize?client_id=482832916175650816&permissions=8&scope=bot');
  }
});
...
client.on('message', msg => {
  if (msg.content === prefix +'support') {
    msg.reply('https://discord.gg/Ju9TaPr');
  }
});
...
client.on('message' , message => {
    ...
    if(message.content.startsWith(adminprefix + "rolebc")) {
...
client.on ("guildMemberAdd", member => {
   ...
   var role = member.guild.roles.find ("name", "عضو");
   member.addRole (role);
...
client.on("message", message => {
 ...
 client.on('message', message => {
           if (message.content.startsWith(prefix + "اي دي")) {
                     if(!message.channel.guild) return message.reply(`هذا الأمر فقط ل السيرفرات ❌`);
